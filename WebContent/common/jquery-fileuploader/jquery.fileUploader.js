	...
	$.fn.fileUploader = function(config){
		...
		var px = {
			...
			init: function(){
				...
				//Wrap input button
				$(e).wrap('<div class="px-input-button" style="height: 28px;" />');
			...
			validateFileName: function(filename) {
				var extensions = new RegExp(config.notAllowedExtension + '$', 'i');
		...
		*/
		function uploadChange($this) {
			...
			if(config.issue_no == ""){
				var issue_no = $form.find(":input[name='in']").val();
				if(issue_no != ""){
					config.issue_no = issue_no;
				}
			}
			...
			//print form
			px.printForm();
			$form.find(":input[name='in']").val(config.issue_no);
		...
		*/
		function html5Upload($form) {
			...
			if (file) {
				...
				.complete(function(jqXHR, textStatus) {
					...
					afterEachUpload($form.attr('id'), $.parseJSON(removeTag(jqXHR.responseText)));
					$progressBar.progressbar({
						value: 100
					});
					$percentage.text('100%');
		...
		function iframeUpload($form) {
			...
			$(id +'_frame').load(function(){
				...
				afterEachUpload($form.attr('id'), $.parseJSON(removeTag(data)) );
