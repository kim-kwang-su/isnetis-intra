...
<c:set var="two_depth" value="고객사별 최종버전"/>
...
<%@ include file="../common/header.jsp" %>
		...
		<div id="content">
			...
			<!-- 네비게이션 -->
			<div id="breadcrumb"><a href="../home.isnet">Home</a> / <a href="module_01.isnet">버전관리</a> / <strong>고객사별 최종버전</strong></div>
			
			<!-- 컨텐츠 표시부 시작 -->
			<!-- 검색 조건 -->
			<div id="searchbox" style="padding:5px 30px;margin:10px 0 0 0;border:1px solid #cecece;">
				<strong>고객사 : </strong>
				<select id="customer_no" style="margin-right: 20px;">
				...
				<strong>제품명 : </strong>
				<select id="product_no" style="margin-right: 20px;">
					<option value="">선택하여 주세요.</option>
				...
				<%-- <strong>설치경로 : </strong>
				<select id="location_no" style="margin-right: 20px;">
...
$(document).ready(function(){
	...
	$("#search_bt").click(function(event){
		...
		var param = {url:"list_03.isnet?cn="+$("#customer_no").val()+"&pn="+$("#product_no").val(),datatype:"json"};
	...
	$("#module_list").jqGrid({
		...
		datatype: "local",
		//url:"list_01.isnet",
		//A.MODULE_NO, B.MODULE_HISTORY_NO, A.MODULE_NAME, B.FILE_VERSION, B.SAVE_PATH, B.UPDATE_MEMO, UPDATE_DATE
	   	colNames:['MODULE_HISTORY_NO', 'MODULE_NO','설치경로', '모듈명','최종 파일버전', '최종수정일자', '변경내용',  'X86_릴리즈', 'X86_디버그','X64_릴리즈', 'X64_디버그' ],						
	   	colModel:[	
	  			{name:'MODULE_HISTORY_NO', index:'MODULE_HISTORY_NO', hidden:true},	 
				{name:'MODULE_NO', index:'MODULE_NO', hidden:true},	   	          		
		   		{name:'FOLDER_PATH',index:'FOLDER_PATH', width:120, align:"left", sortable:false, search:false},
		   		{name:'MODULE_NAME',index:'MODULE_NAME', width:132, align:"center", sortable:false, search:false},
