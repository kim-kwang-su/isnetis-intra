...
<c:set var="two_depth" value="직원별 조회"/>
...
<%@ include file="../common/header.jsp" %>
		...
		<div id="content">
			...
			<!-- 네비게이션 -->
			<div id="breadcrumb"><a href="../home.isnet">Home</a> / <a href="issue_01.isnet">일정관리</a> / <strong>직원별 조회</strong></div>
			
			<!-- 컨텐츠 표시부 시작 -->
			<!-- 검색 조건 -->
			<div id="searchbox" style="padding:5px 30px;margin:10px 0 0 0;border:1px solid #cecece;">
				<strong>부서명 : </strong>
				<select id="dept_no_box" name="DEPT_NO" class="mr20">
				...
				<strong>직원명 : </strong>
				<select id="employee_no_box" name="EMPLOYEE_NO" class="mr5">
					<c:forEach var="employee" items="${employeeList }">
						<c:choose>
							<c:when test="${LOGIN_USER.EMPLOYEE_NO eq employee.EMPLOYEE_NO }">
								<option value="${employee.DEPT_NO }_${employee.EMPLOYEE_NO }" selected="selected" >${employee.EMPLOYEE_NAME }</option>
							</c:when>
							<c:otherwise>
								<option value="${employee.DEPT_NO }_${employee.EMPLOYEE_NO }">${employee.EMPLOYEE_NAME }</option>
							</c:otherwise>
						</c:choose>
					</c:forEach>
			    </select>
...
$(document).ready(function() {
	...
	// 직원목록
	var employee_options = $("#employee_no_box option");
	
	$("#dept_no_box").change(function(){
		
		$("#employee_no_box").html("");
		
		var dept_no = $(this).val();
		if(dept_no == ""){
			$.each(employee_options, function(index, option){
				var html = "";
				html += "<option value='"+$(option).val()+"'>"+option.text+"</option>";
				$("#employee_no_box").append(html);
			});
		}else{
			$.each(employee_options, function(index, option){
				var option_val = $(option).val();
				if(option_val.indexOf(dept_no + "_") == 0){
					var html = "";
					html += "<option value='"+$(option).val()+"'>"+option.text+"</option>";
					$("#employee_no_box").append(html);
				}
	...
	$('#calendar').fullCalendar({
		...
		dayClick: function(selected_date, allDay, jsEvent, view){
			...
			var val = $("#employee_no_box").val();
			var employee_no = val.substring(val.indexOf("_")+1);
			
			if(employee_no != '${LOGIN_USER.EMPLOYEE_NO}'){
				simple_popup("안내", "다른 사용자의 스케줄에서 일정을 등록할 수 없습니다.");
				return false;
			}
				...
				buttons:[
					{
						...
						click: function(event){
							...
							if(employee_no != '${LOGIN_USER.EMPLOYEE_NO}'){
								simple_popup("안내", "다른 사용자의 스케줄에서 일정을 삭제할 수 없습니다.");
								return false;
							}
					...
					},
					{
						...
						click: function(event){
							...
							if(employee_no != '${LOGIN_USER.EMPLOYEE_NO}'){
								simple_popup("안내", "다른 사용자의 스케줄에서 일정을 변경할 수 없습니다.");
								return false;
							}
